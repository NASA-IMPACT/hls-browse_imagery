...
def threshold(in_ar, out_ar, xoff, yoff, xsize, ysize, raster_xsize,
    ...
    low_thresh = math.log({low_thresh})
    high_thresh = math.log({high_thresh})
    low_value = {low_value}
...
def granule_vrt(basename):
    ...
    files = [
        "{}.B01.tif".format(basename),
        "{}.B02.tif".format(basename),
        "{}.B03.tif".format(basename),
    ]
...
def create_gibs_tiles(basename):
    ...
    for g in gibs_tiles:
        ...
        tif_file = "{}_{}.tif".format(basename, gid)
        ...
        d.CreateCopy(tif_file, ds, 0, ["TILED=YES", "COMPRESS=LZW"])
        ds = None
        d = None
