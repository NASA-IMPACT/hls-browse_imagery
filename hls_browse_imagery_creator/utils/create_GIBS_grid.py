...
class gibs_mgrs_intersection:
    ...
    def __init__(self):
        ...
        self.get_S2_input()
        self.get_S2grid_from_kml() if self.s2_params["all_s2_tiles"] == "kml" else self.get_S2grid_from_shp()
        self.GIBS_grid = OrderedDict({"type":"FeatureCollection","features":[]})
        self.get_GIBSgrid()
    ...
    def get_S2grid_from_kml(self):
        ...
        if self.s2_params["write_s2_file"]:
            print(f"Writing the reference S2 HLS grid to {self.s2_params['output_filepath']}")
            with open(self.s2_params["s2_output_filepath"],"w") as f:
                json.dump(self.S2_HLS_grid,f)

    def get_GIBSgrid(self):
        min_lon = self.gibs_params.get("min_lon",-180) ; min_lat = self.gibs_params.get("min_lat",-90)
        max_lon = self.gibs_params.get("max_lon", 180) ; max_lat = self.gibs_params.get("max_lat", 90)
